...
async function trainModel(language) {
    try {
        ...
        console.log(`Training model with ${xTrain.length} samples, sequence length ${maxSeqLen}...`);
        await model.fit(xs, ys, {epochs: 1, batchSize: 8}); // Reduced batch size for memory
...
function editCodeWithNeuralNetwork(code, language) {
    ...
    if (language === 'javascript') {
        ...
        editedCode = editedCode.replace(/border-radius:\s*([^;]+);/g, '-webkit-border-radius: $1;\n    -moz-border-radius: $1;\n    border-radius: $1;');
